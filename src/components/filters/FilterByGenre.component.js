...
import Checkbox from '../layout/Checkbox.component';
import PropTypes from 'prop-types';
import './FilterByGenre.styles.css';
          ...
          <div id='filter-genres' className='film-filter'>
            ...
            <div id='main-genres' onChange={(e) => handleFilterByGenre(e)}>
              {mainGenres.map((g, index) => {
                return <Checkbox key={index} genre={g} genres={genres} />;
              })}
            </div>
            <span
              className={`cta-expand ${this.state.expanded ? 'expanded' : ''}`}
